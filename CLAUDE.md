...
2. **Other agents are executors.** GLM (via Anthropic-compatible endpoint)
   ...
   Code CLI is available only as a premium review escalation.
...
- **All test/eval traffic routes through GLM** (`.env.json`) or `agy` CLI. **Claude Max5 is reserved for orchestration, milestone smoke checks, and explicit premium review escalation.**
