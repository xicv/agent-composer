...
| Codex review explicitly requested, or high-risk/security-sensitive second opinion | `composer_review_claude` directly after the default review gate |
...
model catches more). When the user explicitly asks for Codex review, or the
...
- **`composer_status`** — read-only state snapshot: mode, integrations (review / lifecycle / oracle / git-hook / disabled), the active FOREGROUND worker, active/latest async jobs, latest audit facts, and a recommended next action. Inside Codex prefer this over the `agent-composer status` CLI — only the MCP tool sees live in-session `composer_session_*` overrides.
...
Optional cross-LLM review lane using the OpenAI `codex` Codex plugin:
a different model catches issues agy / Codex miss. OFF by default via
...
When composer-mastermind runs inside a headless `Codex -p` (eval harness,
...
```sh
Codex -p --model Codex-haiku-4-5-20251001 \
...
A headless `Codex -p` worker you spawn to AUTHOR files (multi-file
...
ahead of `Codex -p ...` (the boundary_guard deny message prints the exact
form to use).
