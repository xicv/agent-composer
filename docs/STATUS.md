...
| 2026-06-22 | Slice 3.1 (hot-reload fallback fix — dogfood-found bug: `refreshConfigIfChanged` closure updated roles but not the cached `activeEffectiveFallbacks`, so a profile switched via config-file edit applied role overrides yet silently ignored its runtime `fallbacks` until a server restart or `config_set`; DRY'd the recompute into one `applyActiveConfig` used by both the `ctx.setActiveConfig` and hot-reload `setActiveConfig` paths) | opus-4-8 orch + `composer_code_cli` (codex) | n/a | n/a | n/a | n/a | 1 code | 2 modified | +80/-7 | Found by fork-A dogfood (live global config). RED confirmed independently (test fails without fix), then GREEN: 923/923 vitest, tsc clean (src+test), no schema change. Branch `fix/profile-hotreload-fallbacks` |
| 2026-06-22 | config_set profile switch (#2: `composer_config_set` gains `activeProfile` + `clearActiveProfile` with fail-closed `resolveEffectiveConfig` validation before write; `composer-profile/SKILL.md` corrected from the nonexistent `patch.activeProfile` to the real `{scope, activeProfile}` API) | opus-4-8 orch + `composer_code_cli` (codex) | n/a | n/a | n/a | n/a | 1 code | 3 modified | +154/-5 | Dogfood follow-up #2 (switching now works via the sanctioned MCP tool, not just hand-edits). RED (3 fail) confirmed independently then GREEN: 926/926 vitest, tsc clean (src+test), no schema change. Branch `feat/config-set-active-profile` |
| 2026-06-22 | Profile sample exercisable + docs (#4: `glm-coder` sample fallback fixed from `coderCli->coder` [MUTATE, runtime-inert] to `reviewerClaude->reviewer` [READ, actually fires]; `init`/`init --global` scaffold now seeds the inert sample so fresh configs are discoverable in the config the server reads; composer-profile/SKILL.md gains a "Where Profiles Live" section — config location vs COMPOSER_CONFIG, inert-without-activeProfile, read-vs-mutate fallback reality) | opus-4-8 orch + `composer_code_cli` (codex) | n/a | n/a | n/a | n/a | 1 code | 6 modified | +53/-4 | Dogfood follow-up #4 (last open finding closed). 926/926 vitest, tsc clean (src+test), schema-lint valid. Branch `feat/profile-sample-exercisable` |
