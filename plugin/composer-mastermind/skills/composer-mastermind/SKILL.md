...
- **DO NOT** use `Edit`, `Update`, `Write`, or `NotebookEdit`. For file
  mutations, call `composer_code_cli` directly.
...
- **EXCEPTION — `composer_code_cli`:** call this
  **directly** from the main session for any file create / edit / refactor.
  ...
  The configured CLI executor is Codex on this machine.
- **NEVER** write code in the main session — not even a one-liner or a Bash
  heredoc / `sed` / `awk` rewrite. Use `composer_code_cli`
  so the executor writes off-CC.
...
`composer_code_cli`. Codex must trust that directory
...
`composer_code_cli` (passing `handoffPath`) → `composer_review` on the `git diff` with the same
...
skill+agent registry plus one Task roundtrip. Composer keeps verbose
executor work OUT of the orchestrator context window; it does NOT reduce
total token cost. Dogfood measures a ~2-6x total-token premium vs inline
work, so the headline metric is `mainSessionTokens` kept out of the
calling session. For tiny clarifications / refusals / one-line answers,
**inline is correct**. Heavy work (multi-file code, real research,
multi-step refactors, risky changes) is where dispatch pays through
context preservation, safety, and reviewability. Trust your
...
- **`composer_audit({action:"record"|"read"|"summary"})`** — durable route/outcome trail. After a dispatch, record what happened (route, provider, `reviewVerdict`, `testsPassed`, `userCorrection`, `status`) so `/evolve` learns from real failures, not transcripts. Recording is EXPLICIT — this tool is never auto-called; keep `note`/`objective` short (capped on write). Use `action:"read"` or `action:"summary"` to inspect route accuracy.
